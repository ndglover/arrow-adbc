...
public sealed class SnowflakeDatabase : AdbcDatabase
{
    ...
    public SnowflakeDatabase(IReadOnlyDictionary<string, string>? parameters = null, HttpClient? httpClient = null, ILoggerFactory? loggerFactory = null)
    {
        ...
        _httpClient = httpClient ?? CreateHttpClient(parameters);
    ...
    private static HttpClient CreateHttpClient(IReadOnlyDictionary<string, string>? parameters)
    {
        var skipSslVerify = parameters != null
            && parameters.TryGetValue("adbc.snowflake.sql.ssl_skip_verify", out var value)
