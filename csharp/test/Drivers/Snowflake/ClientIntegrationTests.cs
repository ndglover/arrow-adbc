...
public class ClientIntegrationTests
{
    ...
    {
        var query = $"SELECT * FROM {SampleDataTable} LIMIT {limit}";
        var (totalRows, batchCount, schema, elapsed) = await ExecuteQueryAndReadResultsAsync(query);

        Assert.Equal(limit, totalRows);
        Assert.Equal(totalRows, totalRows); // row count matches

        _output.WriteLine($"Fetched {totalRows} rows in {batchCount} batches with {schema.FieldsList.Count} columns for limit {limit} in {elapsed.TotalMilliseconds:F0} ms.");
    }
